...
/* 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀 Task 3: 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀  
 ...
 function getArtistByIndex(varArray, varIndex) {
  let artistString = `the artist at index ${varIndex} is ${varArray[varIndex].name}`; 
  return artistString;
}  
...
function get20s(varArray){
  const arrayResults = [];
    
  for(let key in varArray){
    let yearString = varArray[key].years;
    let birthYear = Number(varArray[key].years.substring(0,4));
    let deathYear =Number(varArray[key].years.substring(yearString.length-4))
    if(birthYear >=1900 && deathYear <= 2000){
      arrayResults.push(varArray[key].name);
    }
  }
  return arrayResults;
}
...
/* 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀 Task 5: 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀 
 ...
 function removeArtist(varArray, varIndex){
  varArray.splice(varIndex, 1);
...
function addArtist(varArray){
  varArray.push({      
    ...
    "bio": "First sentence. Second sentence"})
    return varArray;
}
...
function lotsOfArt(varArray){
  const resultsArray = []
  for(let key in varArray){
    if(varArray[key].paintings>100){
      resultsArray.push(varArray[key].name);
    }
  }
  return resultsArray;
}
